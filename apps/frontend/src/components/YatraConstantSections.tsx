  ...
  return (
    <section className="px-[7vw] pt-10 pb-10 sm:pt-12 sm:pb-12">
      <div className="mb-3">
        ...
        <blockquote className="mb-10 border-l-2 border-[#7C8A72]/50 pl-6 font-serif text-[clamp(20px,2.6vw,30px)] leading-[1.45] font-normal text-[#2C2A22] max-sm:text-[16px]">
          &ldquo;Every Sacred Walk has been thoughtfully designed to let you focus entirely on the
          spiritual experience while we take care of everything else.&rdquo;
        </blockquote>
        <p className="text-[11.5px] tracking-[0.28em] text-[#7C8A72] uppercase">
          Included throughout your journey
        </p>
      </div>

      <div className="mt-8 grid grid-cols-2 gap-x-8 gap-y-14 sm:grid-cols-3 md:grid-cols-5 md:gap-x-6">
        {EXPERIENCE_ITEMS.map((item) => (
          <div key={item.label} className="flex flex-col items-center gap-5 text-center">
            <span className="flex h-[70px] w-[70px] shrink-0 items-center justify-center rounded-full border border-[#25241E]/70">
              <ExperienceIcon name={item.icon} />
            </span>
            <span className="flex flex-col gap-1">
              {item.sub && (
                <span className="text-[9.5px] tracking-[0.16em] text-[#9A917D] uppercase">
                  {item.sub}
                </span>
              )}
              <span className="font-display text-[12.5px] tracking-[0.06em] text-[#25241E] uppercase">
                {item.label}
              </span>
            </span>
          </div>
...
export function RitualsAndEnhancements() {
  return (
    <section className="bg-[#25241E] px-[7vw] py-16 text-[#F0EADB] sm:py-20">
      ...
      <div className="mt-14 border-t border-[#F5F1E9]/14 pt-10 text-center">
        <p className="text-[13px] tracking-[0.06em] text-[#9CA793] uppercase">
          Important Information
        </p>
        <p className="mx-auto mt-4 max-w-[820px] text-[14px] leading-[1.8] text-[#D8D2C2]">
